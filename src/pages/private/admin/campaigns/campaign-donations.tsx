...
function CampaignDonations({open, setOpen, selectedCampaign} :{
    open: boolean,
    ...
    const getData = async() =>{
        try {
            setLoading(true);
            const response = await axios.get(`/api/donations/get-donations-by-campaign/${selectedCampaign._id}`)
  ...
  return (
  <Modal open={open} className='bg-primary'
